...
plataforma_x = largura - plataforma_largura
...
# Guardaremos a velocidade base para cada estágio.
# Ajuste esses valores conforme desejar para a dificuldade.
ESTAGIOS_DIFICULDADE = [
    {"pontos_necessarios": 0, "velocidade_x_bola": 5, "velocidade_y_bola": -10, "gravidade": 0.5, "velocidade_plataforma": 7},
    {"pontos_necessarios": 10, "velocidade_x_bola": 6, "velocidade_y_bola": -12, "gravidade": 0.6, "velocidade_plataforma": 8}, # Mais negativo
    {"pontos_necessarios": 25, "velocidade_x_bola": 7, "velocidade_y_bola": -14, "gravidade": 0.7, "velocidade_plataforma": 9}, # Mais negativo
    {"pontos_necessarios": 45, "velocidade_x_bola": 8, "velocidade_y_bola": -16, "gravidade": 0.8, "velocidade_plataforma": 10}, # Mais negativo
    {"pontos_necessarios": 70, "velocidade_x_bola": 9, "velocidade_y_bola": -18, "gravidade": 0.9, "velocidade_plataforma": 11}, # Mais negativo
    {"pontos_necessarios": 100, "velocidade_x_bola": 10, "velocidade_y_bola": -20, "gravidade": 1.0, "velocidade_plataforma": 12}  # Mais negativo
]
...
def aplicar_dificuldade():
    ...
    velocidade_bola_y = estagio["velocidade_y_bola"] # Usar o valor inicial de y para cada estágio
...
def reiniciar_posicao_bola():
    ...
    bola_x = largura // 2
    bola_y = altura // 2
    # Reinicia a velocidade da bola para a do estágio atual, não valores fixos
    aplicar_dificuldade() # Garante que a bola começa com a velocidade do estágio atual
    ...
    if estado_jogo == MENU_INICIAL:
        ...
        if (plataforma_y <= bola_y + raio <= plataforma_y + plataforma_altura) and \
           (plataforma_x <= bola_x <= plataforma_x + plataforma_largura):
            velocidade_bola_y = -abs(velocidade_bola_y) # Rebote vertical (garante que sempre suba)
            pontuacao += 1
